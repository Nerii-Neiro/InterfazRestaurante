...
public class EmpleadosDAO {
    ...
    public void crear_empleado(int id_empleado, String nombre, String curp, String rfc, double sueldo, String puesto, String telefono, String nss, String horario, String fecha_ingreso,String password,String email) {
    ...
    public void INSERT(){
        ...
        String query = "INSERT INTO Empleados(nombre,curp,rfc,sueldo,puesto,telefono,nss,horario,fecha_ingreso,password,email) " + "VALUES ('"+nombre+"','"+curp+"','"+rfc+"','"+sueldo+"','"+puesto+"','"+telefono+"','"+nss+"','"+horario+"','"+fecha_ingreso+"','"+password+"','"+email+"')";
    ...
    public void UPDATE(){
        String query = "UPDATE Empleados SET nombre = '"+nombre+"',curp = '"+curp+"',rfc='"+rfc+"',sueldo='"+sueldo+"',puesto='"+puesto+"',telefono='"+telefono+"',nss='"+nss+"',horario='"+horario+"',fecha_ingreso='"+fecha_ingreso+"',password='"+password+"',email='"+email+"' WHERE id_empleado="+id_empleado;
        try{
            Statement stm= conexion.connection.createStatement();
            stm.executeUpdate(query);
        }
    ...
    public Empleado empleado_crear(int em_id_empleado){
        String query = "SELECT * FROM Empleados WHERE  id_empleado="+em_id_empleado;
        try{
            Statement stmt = conexion.connection.createStatement();
            ResultSet result = stmt.executeQuery(query);
            while(result.next()){
//                int id_empleado, String nombre, String curp, String rfc, double sueldo, String puesto, String telefono, String nss, String horario, String fecha_ingreso
                return new Empleado(result.getInt("id_empleado"),result.getString("nombre"),result.getString("curp"),result.getString("rfc"),result.getDouble("sueldo"),result.getString("puesto"),result.getString("telefono"),result.getString("nss"),result.getString("horario"),result.getString("fecha_ingreso"),result.getString("password"),result.getString("email"));
            }
        }
        catch(Exception e){}
