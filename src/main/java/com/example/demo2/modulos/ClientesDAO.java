...
public class ClientesDAO {
    ...
    public Cliente cliente_crear(){
        String query = "SELECT * FROM Clientes ORDER BY  id_cliente DESC LIMIT 1";
        try{
            Statement stmt = conexion.connection.createStatement();
            ResultSet result = stmt.executeQuery(query);
            while(result.next()){
                return new Cliente(result.getInt("id_cliente"),result.getString("nombre"),result.getString("direccion"),result.getString("telefono"),result.getString("email"));
            }
        }
        catch(Exception e){}
