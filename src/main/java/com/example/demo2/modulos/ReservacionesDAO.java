...
public class ReservacionesDAO {
    ...
    public Reservacion reservacion_crear(){
        String query = "SELECT * FROM Reservaciones ORDER BY  id_reservacion DESC LIMIT 1";
        try{
            Statement stmt = conexion.connection.createStatement();
            ResultSet result = stmt.executeQuery(query);
            while(result.next()){
                return new Reservacion(result.getInt("id_reservacion"),result.getString("fecha"),result.getString("hora"),result.getString("observaciones"));
            }
        }
        catch(Exception e){}
