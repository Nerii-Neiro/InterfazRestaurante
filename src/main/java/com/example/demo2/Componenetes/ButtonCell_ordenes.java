...
import com.example.demo2.vistas.ordenes;
...
public class ButtonCell_ordenes extends TableCell<OrdenesDAO, String> {
    ...
    public ButtonCell_ordenes(String a_label){
        this.alabel=a_label;
        ...
        celda_buton.setOnAction(event->{
            OrdenesDAO objeto = this.getTableView().getItems().get(this.getIndex());
           // ClientesDAO objeto = this.getTableView().getItems().get(this.getIndex());
            if(a_label.equals("Editar")){
                //PASO PARA EDITAR
                new ordenes(this.getTableView(),objeto,"Modificar Orden:");
            }
            else{
