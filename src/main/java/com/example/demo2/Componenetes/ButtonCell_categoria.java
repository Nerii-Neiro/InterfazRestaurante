...
import com.example.demo2.vistas.categoria;
...
public class ButtonCell_categoria extends TableCell<CategoriasDAO, String> {
    ...
    public ButtonCell_categoria(String a_label){
        this.alabel=a_label;
        ...
        celda_buton.setOnAction(event->{
            CategoriasDAO objeto = this.getTableView().getItems().get(this.getIndex());
           // ClientesDAO objeto = this.getTableView().getItems().get(this.getIndex());
            if(a_label.equals("Editar")){
                //PASO PARA EDITAR
                new categoria(this.getTableView(),objeto,"Modificar Categoria:");
            }
            else{
