...
import com.example.demo2.vistas.detproducto;
...
public class ButtonCell_detproducto extends TableCell<DetProductoDAO, String> {
    ...
    public ButtonCell_detproducto(String a_label){
        this.alabel=a_label;
        ...
        celda_buton.setOnAction(event->{
            DetProductoDAO objeto = this.getTableView().getItems().get(this.getIndex());
           // ClientesDAO objeto = this.getTableView().getItems().get(this.getIndex());
            if(a_label.equals("Editar")){
                //PASO PARA EDITAR
                new detproducto(this.getTableView(),objeto,"Modificar DetProducto:");
            }
            else{
