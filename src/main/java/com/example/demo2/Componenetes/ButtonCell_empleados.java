...
import com.example.demo2.vistas.empleados;
...
public class ButtonCell_empleados extends TableCell<EmpleadosDAO, String> {
    ...
    public ButtonCell_empleados(String a_label){
        this.alabel=a_label;
        ...
        celda_buton.setOnAction(event->{
            EmpleadosDAO objeto = this.getTableView().getItems().get(this.getIndex());
           // ClientesDAO objeto = this.getTableView().getItems().get(this.getIndex());
            if(a_label.equals("Editar")){
                //PASO PARA EDITAR
                new empleados(this.getTableView(),objeto,"Modificar Empleado:");
            }
            else{
