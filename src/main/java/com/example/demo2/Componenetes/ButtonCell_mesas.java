...
import com.example.demo2.vistas.mesas;
...
public class ButtonCell_mesas extends TableCell<MesasDAO, String> {
    ...
    public ButtonCell_mesas(String a_label){
        this.alabel=a_label;
        ...
        celda_buton.setOnAction(event->{
            MesasDAO objeto = this.getTableView().getItems().get(this.getIndex());
           // ClientesDAO objeto = this.getTableView().getItems().get(this.getIndex());
            if(a_label.equals("Editar")){
                //PASO PARA EDITAR
                new mesas(this.getTableView(),objeto,"Modificar Mesa:");
            }
            else{
