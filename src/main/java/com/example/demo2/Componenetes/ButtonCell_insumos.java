...
import com.example.demo2.vistas.insumos;
...
public class ButtonCell_insumos extends TableCell<InsumosDAO, String> {
    ...
    public ButtonCell_insumos(String a_label){
        this.alabel=a_label;
        ...
        celda_buton.setOnAction(event->{
            InsumosDAO objeto = this.getTableView().getItems().get(this.getIndex());
           // ClientesDAO objeto = this.getTableView().getItems().get(this.getIndex());
            if(a_label.equals("Editar")){
                //PASO PARA EDITAR
                new insumos(this.getTableView(),objeto,"Modificar Insumo:");
            }
            else{
