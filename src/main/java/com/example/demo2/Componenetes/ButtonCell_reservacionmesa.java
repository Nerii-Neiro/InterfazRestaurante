...
import com.example.demo2.vistas.reservacionmesa;
...
public class ButtonCell_reservacionmesa extends TableCell<ReservacionMesaDAO, String> {
    ...
    public ButtonCell_reservacionmesa(String a_label){
        this.alabel=a_label;
        ...
        celda_buton.setOnAction(event->{
            ReservacionMesaDAO objeto = this.getTableView().getItems().get(this.getIndex());
           // ClientesDAO objeto = this.getTableView().getItems().get(this.getIndex());
            if(a_label.equals("Editar")){
                //PASO PARA EDITAR
                new reservacionmesa(this.getTableView(),objeto,"Modificar Reservacion-Mesa:");
            }
            else{
