...
import com.example.demo2.vistas.productos;
...
public class ButtonCell_productos extends TableCell<ProductosDAO, String> {
    ...
    public ButtonCell_productos(String a_label){
        this.alabel=a_label;
        ...
        celda_buton.setOnAction(event->{
            ProductosDAO objeto = this.getTableView().getItems().get(this.getIndex());
           // ClientesDAO objeto = this.getTableView().getItems().get(this.getIndex());
            if(a_label.equals("Editar")){
                //PASO PARA EDITAR
                new productos(this.getTableView(),objeto,"Modificar Producto:");
            }
            else{
