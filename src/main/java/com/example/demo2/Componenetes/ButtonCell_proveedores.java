...
import com.example.demo2.vistas.proveedores;
...
public class ButtonCell_proveedores extends TableCell<ProveedoresDAO, String> {
    ...
    public ButtonCell_proveedores(String a_label){
        this.alabel=a_label;
        ...
        celda_buton.setOnAction(event->{
            ProveedoresDAO objeto = this.getTableView().getItems().get(this.getIndex());
           // ClientesDAO objeto = this.getTableView().getItems().get(this.getIndex());
            if(a_label.equals("Editar")){
                //PASO PARA EDITAR
                new proveedores(this.getTableView(),objeto,"Modificar Proveedor:");
            }
            else{
